export default function EventQualityModal({
  open,
  modal,
  setModal,
  actions,
        ...
        <div className="w-full max-w-lg overflow-hidden rounded-[28px] border border-cyan-200/22 bg-[linear-gradient(165deg,rgba(7,38,44,0.9),rgba(11,11,11,0.98))] shadow-[0_28px_120px_rgba(0,0,0,0.58)]">
          ...
          <div className="space-y-4 px-5 py-5">
            <div className="grid gap-2 sm:grid-cols-3">
              {actions.map((item) => {
                const active = modal.action === item.value;
                return (
                  <button
                    key={item.value}
                    type="button"
                    onClick={() => setModal((current) => ({ ...current, action: item.value }))}
            ...
            <div>
              ...
              <input
                ...
                value={modal.sourceInput}
                onChange={(event) => setModal((current) => ({ ...current, sourceInput: event.target.value }))}
