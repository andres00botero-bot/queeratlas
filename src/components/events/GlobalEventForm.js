...
export default function GlobalEventForm({
  ...
  return (
    <form onSubmit={onSubmit} className="mt-6 grid gap-3 md:grid-cols-2">
      ...
      />
      <label className="block rounded-2xl border border-emerald-200/22 bg-emerald-200/[0.08] p-3 md:col-span-2">
        ...
        <input
          value={globalForm.ticket_url || ""}
          onChange={(event) => setGlobalForm((current) => ({ ...current, ticket_url: event.target.value }))}
