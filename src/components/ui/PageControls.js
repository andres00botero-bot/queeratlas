...
export default function PageControls({
  ...
  const isEditorialRail = variant === "editorial-rail" || variant === "events-compact";
  ...
  const rootClassName = isEditorialRail
  ...
  const railClassName = isEditorialRail
  ...
  return (
    ...
    >
      <div className={`pointer-events-none absolute inset-x-5 top-0 h-px bg-gradient-to-r from-transparent via-cyan-200/65 to-transparent ${mobileCompact ? "hidden sm:block" : ""} ${isEditorialRail ? "!hidden" : ""}`} />
      <div className={`pointer-events-none absolute -left-16 top-0 h-32 w-32 rounded-full bg-cyan-300/12 blur-3xl ${mobileCompact ? "hidden sm:block" : ""} ${isEditorialRail ? "!hidden" : ""}`} />
      <div className={`pointer-events-none absolute -right-16 top-0 h-32 w-32 rounded-full bg-fuchsia-300/12 blur-3xl ${mobileCompact ? "hidden sm:block" : ""} ${isEditorialRail ? "!hidden" : ""}`} />
      <div className={`relative mb-3 hidden border-b border-white/12 pb-2.5 sm:mb-3.5 sm:items-end sm:justify-between sm:gap-3 sm:pb-3 ${isEditorialRail ? "sm:hidden" : "sm:flex"}`}>
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-white sm:tracking-[0.2em]">
            Page controls
          </p>
      ...
      >
        {buttons.map((button, index) => {
          ...
          const activeBaseClass = isEditorialRail
          ...
          const mobileButtonLayoutClass = isEditorialRail
          ...
          const controlProps = {
            ...
            className: `${mobileButtonLayoutClass} outline-none transition-[box-shadow,transform,background-color,border-color,color] duration-150 ease-out ${
              isActive
                ? `${activeBaseClass} ${activeClassName}`
                : isEditorialRail
                  ...
                  : "border-white/18 bg-white/8 text-white/84 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] hover:-translate-y-[1px] hover:border-cyan-100/36 hover:bg-white/12 hover:text-white hover:shadow-[0_10px_24px_rgba(34,211,238,0.08)] focus-visible:border-cyan-200/60 focus-visible:text-cyan-50 focus-visible:shadow-[0_0_0_1px_rgba(125,211,252,0.42)]"
            }`,
          };
