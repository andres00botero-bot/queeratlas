...
const TONE_STYLES = {
  ok: {
    shell: "border-emerald-200/30 bg-emerald-200/12 text-emerald-100",
    chip: "border-emerald-200/30 bg-emerald-200/14 text-emerald-50",
    label: "Saved",
  },
  warn: {
    shell: "border-amber-200/30 bg-amber-200/12 text-amber-100",
    chip: "border-amber-200/30 bg-amber-200/14 text-amber-50",
    label: "Needs action",
  },
  info: {
    shell: "border-cyan-200/30 bg-cyan-200/12 text-cyan-100",
  ...
  return (
    <div className={`pointer-events-none fixed z-[70] ${positionClass}`}>
      <div
        role="status"
        aria-live="polite"
        className={`max-w-[92vw] animate-[fadeIn_.22s_ease] rounded-2xl border px-4 py-3 text-xs shadow-[0_24px_55px_rgba(0,0,0,0.45)] backdrop-blur ${tone.shell}`}
      >
