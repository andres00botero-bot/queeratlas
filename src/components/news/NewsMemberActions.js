  ...
  if (mode === "save") {
    return (
      ...
      >
        ...
        {storySaved ? t("now.saved", "Saved") : t("now.save", "Save")}
      </button>
    );
  }

  return (
    <section className="mt-10 border-b border-white/10 pb-8" aria-label={t("now.newsMemberActions", "News member actions")}>
      <p className="text-[10px] font-semibold uppercase tracking-[0.17em] text-white/38">{t("now.keepFollowingStory", "Keep following the story")}</p>
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {showSave ? <button
          ...
          >
            <Bookmark size={15} aria-hidden="true" fill={storySaved ? "currentColor" : "none"} />
            {storySaved ? t("now.storySaved", "Story saved") : t("now.saveStory", "Save story")}
          </button> : null}
        {showFollow && canFollowCity ? (
          ...
          >
            ...
            {cityFollowed ? t("now.followingCity", "Following {city}").replace("{city}", article.city) : t("now.followCity", "Follow {city}").replace("{city}", article.city)}
          </button>
        ) : null}
        {showFollow ? <button
          type="button"
        ...
        >
          ...
          {topicFollowed ? t("now.followingTopic", "Following {topic}").replace("{topic}", article.storyType) : t("now.followTopic", "Follow {topic}").replace("{topic}", article.storyType)}
        </button> : null}
      </div>
      {!cloudAvailable ? <p className="mt-2 text-[11px] text-amber-100/58">{t("now.localSavedNotice", "Saved on this device. Cloud sync activates when the News preferences migration is installed.")}</p> : null}
    </section>
  );
}
