...
export default function AddPlaceInlineForm({
  ...
  return (
    <div className="mb-6 space-y-3 rounded-[28px] border border-emerald-300/12 bg-[linear-gradient(180deg,rgba(9,36,30,0.92),rgba(14,14,14,0.96))] p-5 shadow-[0_18px_50px_rgba(16,185,129,0.08)]">
      <input value={name} onChange={(event) => setName(event.target.value)} placeholder={t("city.placeName", "Place name")} className="w-full rounded-2xl border border-white/10 bg-black/30 p-3 outline-none" />
      <textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder={t("city.shortDescription", "Short description (vibe, crowd, energy...)")} className="w-full rounded-2xl border border-white/10 bg-black/30 p-3 outline-none" />
      {type !== "store" && (
        <VibeTagPicker
          value={vibeTags}
          ...
          tone="emerald"
          title={t("city.venueVibeTags", "Venue vibe tags")}
          hint={t("city.venueVibeTagsHint", "Choose up to 3 tags for standardized discovery.")}
        />
      )}
      <input
        value={vibe}
        onChange={(event) => setVibe(event.target.value)}
        placeholder={t("city.legacyVibe", "Legacy vibe label (optional)")}
        className="w-full rounded-2xl border border-white/10 bg-black/30 p-3 outline-none"
      />
      <input value={placeHours} onChange={(event) => setPlaceHours(event.target.value)} placeholder={t("city.openingHours", "Opening hours (for example Thu-Sat 22:00-05:00)")} className="w-full rounded-2xl border border-white/10 bg-black/30 p-3 outline-none" />
      <input value={placeLink} onChange={(event) => setPlaceLink(event.target.value)} placeholder={t("city.officialLink", "Official link (website, Instagram, Facebook) - optional")} className="w-full rounded-2xl border border-white/10 bg-black/30 p-3 outline-none" />
      <VenueLocationPicker
        address={address}
        city={city}
      ...
      <PracticalIntelFields
        title={t("city.practicalVenueIntel", "Practical venue intelligence")}
        description={t("city.practicalVenueIntelDescription", "Optional first-hand details that help people choose when and how to visit. The prompts adapt to the venue type.")}
        fields={[
