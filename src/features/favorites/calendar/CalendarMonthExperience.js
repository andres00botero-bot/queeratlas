...
export default function CalendarMonthExperience({
  ...
  const monthLabel = useMemo(
    () => new Date(`${monthKey}-01T12:00:00`).toLocaleDateString(locale === "es" ? "es" : "en-US", { month: "long", year: "numeric" }),
    [locale, monthKey]
  );
    ...
    <>
      <div className="overflow-hidden rounded-[30px] border border-white/10 bg-[radial-gradient(circle_at_12%_0%,rgba(245,169,198,0.09),transparent_31%),linear-gradient(180deg,#19131f_0%,#120e17_100%)] shadow-[0_28px_82px_rgba(0,0,0,0.32)]">
        <header className="flex flex-col gap-4 border-b border-white/8 px-4 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <div>
            <p className="text-[10px] uppercase tracking-[0.19em] text-[#f5a9c6]/68">{t("favorites.monthOverview", "Month overview")}</p>
          ...
          <div className="grid w-full grid-cols-[minmax(0,1fr)_auto] gap-2 sm:flex sm:w-auto sm:items-center sm:justify-end">
            <div className="flex min-w-0 items-center justify-between rounded-full border border-white/9 bg-white/[0.03] p-1 sm:justify-start">
              <button type="button" onClick={() => onMoveMonth(-1)} aria-label={t("favorites.previousMonth", "Previous month")} className="flex h-10 w-10 items-center justify-center rounded-full text-white/58 transition hover:bg-white/[0.07] hover:text-white"><ChevronLeft size={18} /></button>
              <button type="button" onClick={onToday} className="min-h-10 rounded-full px-3 text-xs font-semibold text-[#ffd8e7] transition hover:bg-[#f5a9c6]/9">{t("favorites.today", "Today")}</button>
              <button type="button" onClick={() => onMoveMonth(1)} aria-label={t("favorites.nextMonth", "Next month")} className="flex h-10 w-10 items-center justify-center rounded-full text-white/58 transition hover:bg-white/[0.07] hover:text-white"><ChevronRight size={18} /></button>
            </div>
            <button type="button" onClick={onEnablePush} disabled={pushState === "loading" || pushState === "enabled"} className="flex min-h-11 items-center gap-2 rounded-full border border-white/10 bg-white/[0.035] px-3 text-xs font-semibold text-white/62 disabled:opacity-60"><Bell size={15} /> {pushState === "enabled" ? t("favorites.on", "On") : t("favorites.reminders", "Reminders")}</button>
            <button type="button" onClick={openAdd} className="col-span-2 flex min-h-11 w-full items-center justify-center gap-2 rounded-full bg-[#f5a9c6] px-4 text-xs font-semibold text-[#24131d] shadow-[0_10px_28px_rgba(245,169,198,0.18)] transition hover:bg-[#ffc0d7] sm:w-auto"><Plus size={16} /> {t("favorites.addPlan", "Add plan")}</button>
          </div>
        </header>
        ...
        <div className="p-3 sm:p-5">
          <div className="grid grid-cols-7 text-center text-[9px] font-semibold uppercase tracking-[0.12em] text-white/34 sm:text-[10px]">
            {[t("favorites.mon", "Mon"), t("favorites.tue", "Tue"), t("favorites.wed", "Wed"), t("favorites.thu", "Thu"), t("favorites.fri", "Fri"), t("favorites.sat", "Sat"), t("favorites.sun", "Sun")].map((day) => <div key={day} className="py-2">{day}</div>)}
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2 hidden space-y-1 sm:block">
                    ...
                    {entries.length > 2 ? <span className="block pl-1 text-[9px] text-white/34">+{entries.length - 2} {t("favorites.more", "more")}</span> : null}
                  </div>
                </button>
              );
            })}
          ...
          <div className="mt-4 flex flex-wrap items-center gap-4 px-1 text-[10px] text-white/42">
            {[{ label: t("favorites.events", "Events"), tone: "bg-[#ff78ad]" }, { label: t("favorites.trips", "Trips"), tone: "bg-[#88d9d4]" }, { label: t("favorites.personal", "Personal"), tone: "bg-[#d8b678]" }].map((item) => <span key={item.label} className="flex items-center gap-1.5"><span className={`h-1.5 w-1.5 rounded-full ${item.tone}`} />{item.label}</span>)}
          </div>
        </div>
      </div>

      {isDayOpen ? (
        <Sheet title={new Date(`${selectedDateKey}T12:00:00`).toLocaleDateString(locale === "es" ? "es" : "en-US", { weekday: "long", month: "long", day: "numeric" })} eyebrow={t("favorites.yourDay", "Your day")} onClose={() => setIsDayOpen(false)}>
            ...
            {selectedEntries.length ? selectedEntries.map((entry) => {
              ...
              return (
                <button key={entry.id} type="button" onClick={() => { setActiveEntry(entry); setIsDayOpen(false); }} className="relative flex min-h-[4.75rem] w-full items-center gap-3 overflow-hidden rounded-[18px] border border-white/9 bg-white/[0.035] px-4 py-3 text-left transition hover:border-white/16 hover:bg-white/[0.055]">
                  ...
                  <span className="w-12 flex-none text-center text-xs font-semibold text-white/66">{entry.time || t("favorites.allDay", "All day")}</span>
                  <span className="min-w-0 flex-1 border-l border-white/8 pl-3"><span className={`block text-[9px] uppercase tracking-[0.13em] ${tone.text}`}>{entry.status === "going" ? t("favorites.going", "Going") : entry.kind === "event" ? t("favorites.savedEvent", "Saved event") : entry.kind === "plan" ? t("favorites.tripPlan", "Trip plan") : t("favorites.personal", "Personal")}</span><span className="mt-1 block truncate text-sm font-semibold text-[#fff7fb]">{entry.title}</span><span className="mt-1 block truncate text-xs text-[#bcaeb9]">{entry.city || t("favorites.locationNotSet", "Location not set")}</span></span>
                </button>
              );
            }) : <div className="rounded-[20px] border border-dashed border-white/12 px-5 py-9 text-center"><CalendarDays className="mx-auto text-[#f5a9c6]/58" size={26} /><p className="mt-3 text-sm text-[#bcaeb9]">{t("favorites.nothingPlanned", "Nothing planned yet.")}</p></div>}
          </div>
          <button type="button" onClick={openAdd} className="mt-5 flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-[#f5a9c6] text-sm font-semibold text-[#24131d]"><Plus size={16} /> {t("favorites.addToDay", "Add something to this day")}</button>
        </Sheet>
      ) : null}

      {activeEntry ? (
        <Sheet title={activeEntry.title} eyebrow={activeEntry.kind === "event" ? t("favorites.savedEvent", "Saved event") : activeEntry.kind === "plan" ? t("favorites.tripPlan", "Trip plan") : t("favorites.personal", "Personal")} onClose={() => setActiveEntry(null)}>
          <div className="mt-5 flex flex-wrap gap-2 text-xs text-[#bcaeb9]">
            ...
            <span className="flex items-center gap-1.5 rounded-full border border-white/9 bg-white/[0.035] px-3 py-2"><MapPin size={14} /> {activeEntry.city || t("favorites.locationNotSet", "Location not set")}</span>
          ...
          <div className="mt-6 grid gap-2 sm:grid-cols-2">
            {activeEntry.kind === "event" ? <>
              <button type="button" onClick={() => { onToggleGoing(activeEntry.sourceId); setActiveEntry((current) => ({ ...current, status: current.status === "going" ? "saved" : "going" })); }} className={`flex min-h-12 items-center justify-center gap-2 rounded-full px-4 text-sm font-semibold ${activeEntry.status === "going" ? "border border-[#f5a9c6]/28 bg-[#f5a9c6]/12 text-[#ffd8e7]" : "bg-[#f5a9c6] text-[#24131d]"}`}><Sparkles size={15} /> {activeEntry.status === "going" ? t("favorites.going", "Going") : t("favorites.markGoing", "Mark as going")}</button>
              <button type="button" onClick={() => onOpenEvent(activeEntry)} className="min-h-12 rounded-full border border-white/12 bg-white/[0.045] px-4 text-sm font-semibold text-white/76">{t("favorites.openEvent", "Open event")}</button>
              ...
              <button type="button" onClick={() => onAddToTrip(activeEntry)} className="min-h-12 rounded-full border border-[#88d9d4]/20 bg-[#88d9d4]/9 px-4 text-sm font-semibold text-[#bff5ef]">{t("favorites.addToTrip", "Add to trip")}</button>
      ...
      {isAddOpen ? (
        <Sheet title={t("favorites.addPlan", "Add plan")} eyebrow={t("favorites.planEyebrow", "A little something to look forward to")} onClose={() => setIsAddOpen(false)}>
          <form onSubmit={submitItem} className="mt-6 grid gap-3">
            <label className="grid gap-1.5 text-xs text-white/54">{t("favorites.title", "Title")}<input required autoFocus value={itemForm.title} onChange={(event) => setItemForm((current) => ({ ...current, title: event.target.value }))} placeholder={t("favorites.planPlaceholder", "Dinner, meetup, show…")} className="min-h-12 rounded-[16px] border border-white/11 bg-black/20 px-4 text-sm text-white outline-none placeholder:text-white/28 focus:border-[#f5a9c6]/44" /></label>
            ...
            <button type="submit" className="mt-2 min-h-12 rounded-full bg-[#f5a9c6] px-5 text-sm font-semibold text-[#24131d] shadow-[0_12px_30px_rgba(245,169,198,0.18)]">{t("favorites.savePlan", "Save plan")}</button>
          </form>
        </Sheet>
      ) : null}
    </>
