...
export async function generateMetadata() {
  ...
  if (locale !== "es") return { ...metadata, alternates: localizedAlternates("/now/collections", locale), openGraph: { ...metadata.openGraph, url: localizedOpenGraphUrl("/now/collections", locale), locale: "en_US" } };

  return {
    ...metadata,
    alternates: localizedAlternates("/now/collections", locale),
    title: "Colecciones Atlas | Mejores viajes, estancias, eventos y cultura queer",
    description: "Explora colecciones de viaje LGBTQ+ investigadas sobre alojamientos queer, lunas de miel, escapadas urbanas asequibles, viajes sáficos, eventos, bienestar, comida, vida nocturna y playas.",
    openGraph: {
      ...metadata.openGraph,
      url: localizedOpenGraphUrl("/now/collections", locale),
      locale: "es_ES",
      title: "Colecciones Atlas | Queer Atlas",
      description: "Lugares queer, estancias, eventos y rutas urbanas seleccionados por cómo funcionan realmente como viaje.",
    },
  };
}
...
function collectionFilterLabel(filter, t) {
  return t(`now.collectionFilter.${filter.id}`, filter.label);
}
...
function CollectionCover({ collection, index, lead = false, t }) {
  ...
  return (
    <div className={`relative overflow-hidden border-b border-white/10 bg-[#0b0e15] ${lead ? "min-h-64 sm:min-h-72" : "min-h-40"}`} aria-hidden="true">
      ...
      <div className="absolute inset-x-5 top-4 flex items-center justify-between text-[9px] font-semibold uppercase tracking-[0.18em] text-white/48 sm:inset-x-6">
        <span>{t("now.collectionFieldEdit", "Atlas field edit")}</span>
...
function CollectionCard({ collection, index, t }) {
  ...
  return (
    <article className={`group flex h-full flex-col overflow-hidden rounded-[24px] border border-white/11 bg-[linear-gradient(180deg,rgba(15,18,27,0.98),rgba(8,10,15,0.99))] shadow-[0_18px_55px_rgba(0,0,0,0.18)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_28px_75px_rgba(0,0,0,0.30)] ${style.border}`}>
      <CollectionCover collection={collection} index={index} t={t} />
      <div className="flex flex-1 flex-col p-5">
        ...
        <dl className="mt-5 grid grid-cols-2 gap-x-4 border-y border-white/9 py-3">
          <div>
            <dt className="text-[9px] font-semibold uppercase tracking-[0.13em] text-white/34">{t("now.inside", "Inside")}</dt>
            <dd className="mt-1 text-xs text-white/72">{t("now.picksCount", "{count} picks").replace("{count}", collection.items.length)}</dd>
          </div>
          <div>
            <dt className="text-[9px] font-semibold uppercase tracking-[0.13em] text-white/34">{t("now.verified", "Verified")}</dt>
    ...
    <main className="qa-page min-h-screen bg-[radial-gradient(circle_at_7%_0%,rgba(45,212,191,0.09),transparent_25%),radial-gradient(circle_at_91%_3%,rgba(251,191,36,0.07),transparent_24%),linear-gradient(180deg,#07090e_0%,#090b11_48%,#06070a_100%)] px-4 py-6 text-white sm:px-6 sm:py-9">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <header className="relative mt-5 overflow-hidden border-y border-white/10 py-7 sm:py-9">
          ...
          <div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <div className="max-w-3xl">
              <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-emerald-100/66">{t("now.collectionDesk", "Queer Atlas field desk")}</p>
              <h1 className="mt-3 text-4xl font-black leading-none tracking-[-0.045em] text-[#f7f4ee] sm:text-5xl">{t("now.collections", "Atlas Collections")}</h1>
        ...
        <section id="discover-collections" className="scroll-mt-24 pt-6 sm:scroll-mt-8 sm:pt-8" aria-labelledby="collections-discovery-heading">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-100/58">{t("now.browseFieldDesk", "Browse the field desk")}</p>
              <h2 id="collections-discovery-heading" className="mt-2 text-2xl font-black tracking-[-0.04em] text-[#f7f4ee] sm:text-3xl">{t("now.findTripEdit", "Find an edit for this trip")}</h2>
            </div>
          ...
          <form action="/now/collections" className="mt-5 grid gap-2 border-y border-white/9 py-4 sm:grid-cols-[minmax(0,1fr)_11rem_auto] sm:items-center">
            ...
            <label htmlFor="collection-sort" className="sr-only">{t("now.sortCollections", "Sort collections")}</label>
            <select id="collection-sort" name="sort" defaultValue={validSort} className="min-h-12 rounded-xl border border-white/11 bg-[#10131b] px-3 text-sm text-white/74 outline-none focus:border-cyan-100/38">
              <option value="curated">{t("now.curatedOrder", "Curated order")}</option>
              <option value="verified">{t("now.recentlyVerified", "Recently verified")}</option>
              <option value="az">{t("now.alphabetical", "A–Z")}</option>
            </select>
        ...
        <footer className="mt-12 flex flex-col gap-4 border-t border-white/9 py-7 sm:flex-row sm:items-center sm:justify-between">
          <p className="max-w-2xl text-xs leading-5 text-white/40">{t("now.collectionFooter", "Every edit distinguishes dedicated queer spaces from recurring events and records when its selections were last reviewed.")}</p>
          <div className="flex gap-4 text-xs font-semibold text-white/54">
            <Link href="/editorial-policy" className="hover:text-white">{t("now.editorialPolicy", "Editorial policy")}</Link>
            <Link href="/sources-and-reviews" className="hover:text-white">{t("now.sourcesReviews", "Sources & reviews")}</Link>
          </div>
        </footer>
      </div>
    </main>
